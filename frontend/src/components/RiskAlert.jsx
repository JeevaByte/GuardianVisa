    ...
    <div className={`rounded-xl border-2 ${cfg.bg} ${cfg.border} overflow-hidden shadow-2xl ${cfg.pulse ? 'animate-pulse' : ''}`}>
      ...
      <div className="bg-slate-800 px-5 py-5 space-y-5">
        ...
        {data.hours_analysis && (
          <div className="bg-slate-900 rounded-lg p-4 border border-slate-700">
            ...
            <div className="text-2xl font-extrabold text-white font-mono tracking-tight">
              {data.hours_analysis.current_hours} hrs this week
              {data.hours_analysis.proposed_hours != null && (
                <>
                  <span className="text-slate-400 mx-2">+</span>
                  <span className="text-amber-400">{data.hours_analysis.proposed_hours} proposed</span>
                  <span className="text-slate-400 mx-2">=</span>
                  <span className={isViolation ? 'text-red-400' : 'text-green-400'}>
                    {(data.hours_analysis.current_hours || 0) + (data.hours_analysis.proposed_hours || 0)} hrs
                  </span>
                </>
              )}
            </div>
            <p className="text-slate-400 text-sm mt-1">
              Legal limit:{' '}
              <span className="font-bold text-white">{data.hours_analysis.limit} hrs/week</span>
              {isViolation && data.hours_analysis.proposed_hours != null && (
                <span className="ml-2 text-red-400 font-bold">
                  ({((data.hours_analysis.current_hours || 0) + (data.hours_analysis.proposed_hours || 0)) - data.hours_analysis.limit} hrs over limit!)
                </span>
              )}
            </p>
          </div>
        ...
        {data.safe_response && (
          <div className="bg-green-950/40 border border-green-800 rounded-lg p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <CopyButton text={data.safe_response} />
            </div>
        ...
        {data.safe_alternatives && data.safe_alternatives.length > 0 && (
          <Collapsible title={`💡 Safe Alternatives (${data.safe_alternatives.length})`}>
